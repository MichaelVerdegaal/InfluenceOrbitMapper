...
templates = {'orbit_test': [1, 4148],  # Used to confirm correct orbit calculations via game website comparisons
             'demo': [1, 250000, 104, 235773, 13, 4148],  # Default asteroid choices
...
while len(rocks) < 9:
    ...
    if asteroid_number.isnumeric() and (0 < int(asteroid_number) <= 250000):
        ...
        rocks = [get_roid(roids, rock_id) for rock_id in templates[asteroid_number]]
        break
    else:
        break

if not rocks:
    rocks = [get_roid(roids, rock_id) for rock_id in templates['demo']]
